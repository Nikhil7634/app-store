...
// Fetch existing video data if id is provided for editing
if (isset($_GET['id'])) {
    $videoId = $_GET['id'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $embedCode = $_POST['embed_code'] ?? ''; // This is the embed code (e.g., YouTube iframe)
    $imagePath = $video['image_path']; // Keep the existing image if not updated
    ...
    $sql = "UPDATE youtube SET title = :title, description = :description, embed_code = :embed_code, image_path = :image_path WHERE id = :id";
    $stmt = $dbh->prepare($sql);
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        <div class="grid grid-cols-12 gap-x-4">
            <div class="col-span-full lg:col-span-7 card">
                <div class="p-1.5">
                    <h6 class="card-title">Edit Video Details</h6>
                    ...
                    <div class="mt-7 pt-0.5">
                        <div class="grid grid-cols-full gap-x-4 gap-y-5">
                            <div class="col-span-full xl:col-auto leading-none">
                                ...
                                <input type="text" id="courseTitle" name="title" value="<?php echo htmlspecialchars($video['title']); ?>" placeholder="Title" class="form-input" required>
                            </div>

                            <div class="col-span-full">
                                <label for="description" class="form-label">Description</label>
                                <textarea id="description" name="description" rows="8" class="summernote form-input" required><?php echo htmlspecialchars($video['description']); ?></textarea>
                            </div>

                            <div class="col-span-full">
                                <label class="form-label">Insert Embed Code</label>
                                <textarea name="embed_code" rows="6" class="form-input " placeholder="Enter your Embed Code here..."><?php echo htmlspecialchars($video['embed_code']); ?></textarea>
                            </div>
                        </div>
                    </div>
                </div>
